...
const knowledgePath = path.join(__dirname, "data", "cyber_syndrome_knowledge.json");
...
const preparedChunks = corpus.chunks.map((chunk, index) => ({
  ...
  searchText: normalizeSearchText(
    `${chunk.sourceTitle} ${chunk.fileName} ${chunk.text}`,
  ),
}));
...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    const providerId = String(req.body.provider || "deepseek");
    const provider = providerConfigs[providerId];
    if (!provider) {
      return res.status(400).json({ error: "未知模型供应商。" });
    }

    if (!provider.apiKey) {
      return res.status(400).json({
        error: `尚未配置 ${provider.apiKeyName}。请在 .env 中填写密钥后重启服务。`,
      });
    }
    ...
    if (!lastUserMessage?.content) {
      return res.status(400).json({ error: "请输入一个问题。" });
    }
    ...
    const completion = await callChatCompletion(provider, body, abortController.signal);
    const answer = completion.choices?.[0]?.message?.content?.trim();
    if (!answer) {
      return res.status(502).json({ error: "模型没有返回可显示的回答。" });
    }
    ...
    if (abortController.signal.aborted || error.name === "AbortError") {
      if (!res.headersSent && !res.writableEnded && !res.destroyed) {
        res.status(499).json({ error: "已停止生成。" });
      }
      return;
    }
    console.error(error);
...
app.listen(port, "0.0.0.0", () => {
  console.log(`Cyber-Syndrome consult app is running.`);
  console.log(`Desktop: http://localhost:${port}`);
  console.log(`Mobile QR target: ${publicUrl()}`);
});
...
function localLanAddress() {
  ...
  return (
    ...
    candidates.find((address) => !address.startsWith("169.254.") && !address.startsWith("198.18.")) ||
    candidates[0] ||
    "localhost"
  );
}
...
function publicUrl() {
  ...
  return `http://${localLanAddress()}:${port}`;
}
...
async function callChatCompletion(provider, body, signal) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`${provider.label} 请求失败：${detail}`);
  }
